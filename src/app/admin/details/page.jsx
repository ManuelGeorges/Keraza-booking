...
export default function AdminChurchDetailsPage() {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const churchCompsMap = {};
        churchCompsSnap.forEach(doc => churchCompsMap[doc.id] = doc.data().competitions || {});

        const otherCompsMap = {};
        otherCompsSnap.forEach(doc => otherCompsMap[doc.id] = doc.data().competitions || {});

        const all = churches.map(name => {
          const comps1 = churchCompsMap[name] || {};
          const comps2 = otherCompsMap[name] || {};
          const allComps = { ...comps1, ...comps2 };

          let totalBeforeDiscount = 0;
          const competitions = Object.entries(allComps).map(([key, value]) => {
          ...
          const discount = settingsMap[name]?.discountPercentage || 0;
          return {
            name,
            competitions,
            totalBeforeDiscount,
            discountPercentage: discount,
            totalAfterDiscount: calculatePriceAfterDiscount(totalBeforeDiscount, discount),
            leaderName: settingsMap[name]?.leader || "---"
          };
        });
  ...
  const handleDiscountChange = async (churchName, newDiscountValue) => {
    let discount = Math.min(100, Math.max(0, parseFloat(newDiscountValue) || 0));
    try {
      await setDoc(doc(db, "churches", churchName), { discountPercentage: discount }, { merge: true });
      setChurchData(prev => prev.map(c => {
        if (c.name === churchName) {
          return {
            ...c,
            discountPercentage: discount,
            totalAfterDiscount: calculatePriceAfterDiscount(c.totalBeforeDiscount, discount)
          };
        }
        return c;
      }));
  ...
  const downloadExcel = () => {
    ...
    filteredChurches.forEach(church => {
      exportData.push({ "الكنيسة": church.name, "الخادم المسؤول": church.leaderName });
      church.competitions.forEach(comp => {
        exportData.push({
          "المسابقة": comp.name,
          "عدد المشاركين": comp.count,
          "السعر الكلي": comp.totalPrice
        });
      });
      exportData.push({
        "المسابقة": "إجمالي الكنيسة",
        "نسبة الخصم": church.discountPercentage,
        "إجمالي قبل الخصم": church.totalBeforeDiscount,
        "إجمالي بعد الخصم": church.totalAfterDiscount
      });
      exportData.push({});
    });
  ...
  return (
    <div className="admin-details-container page-transition">
      ...
      <nav className="church-nav glass-card">
        {filteredChurches.map((c, idx) => (
          <button
            key={idx}
            onClick={() => document.getElementById(`church-${idx}`)?.scrollIntoView({ behavior: "smooth" })}
            className="nav-chip"
          >
            {c.name}
          </button>
      ...
      <div className="church-sections">
        {filteredChurches.map((church, idx) => {
          ...
          return (
            <div key={idx} id={`church-${idx}`} className="church-full-card glass-card">
              <div className="church-card-header-main">
                <div className="header-info">
                  <Church size={32} className="primary-icon" />
                  <div>
                    <h2>{church.name}</h2>
                    <p className="leader-info">الخادم المسؤول: {church.leaderName}</p>
                  </div>
                </div>
                ...
                <div className="discount-control">
                  ...
                  <div className="input-with-percent">
                    <input
                      ...
                      onChange={(e) => handleDiscountChange(church.name, e.target.value)}
                ...
                <table className="church-table">
                  ...
                  <tbody>
                    {church.competitions.map((c, i) => (
                      <tr key={i}><td>{c.name}</td><td className="center-td">{c.count}</td><td className="price-td">{c.totalPrice.toLocaleString()} جـ</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>

              <div className="church-summary-row">
                <div className="summary-item">
                  <span>الإجمالي قبل الخصم:</span>
                  <strong>{church.totalBeforeDiscount.toLocaleString()} جـ</strong>
                </div>
                <div className="summary-item highlighted">
                  <span>الإجمالي بعد الخصم:</span>
                  <strong>{Math.round(church.totalAfterDiscount).toLocaleString()} جـ</strong>
                </div>
              </div>

              <div className="church-charts-container">
                <div className="church-chart">
                  <h3>نسبة المشتركين</h3>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={dataForCount} dataKey="count" nameKey="name" outerRadius={getResponsiveRadius()} innerRadius={60} paddingAngle={5}>
                        {dataForCount.map((_, index) => (
                ...
                <div className="church-chart">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={dataForPrice} dataKey="totalPrice" nameKey="name" outerRadius={getResponsiveRadius()} innerRadius={60} paddingAngle={5}>
                        {dataForPrice.map((_, index) => (
